...
async fn sync_native() {
    ...
    let instruction = tpl_token_interface::instruction::sync_native_with_rent_sysvar(
        &TOKEN_PROGRAM_ID,
        &source_account_key,
    )
    .unwrap();
    ...
    result.resulting_accounts.iter().for_each(|(key, account)| {
        if *key == source_account_key {
            let token_account = tpl_token_interface::state::Account::unpack(&account.data).unwrap();
...
fn sync_native_with_rent_change() {
    ...
    let instruction =
        tpl_token_interface::instruction::sync_native(&TOKEN_PROGRAM_ID, &source_account_key)
            .unwrap();
