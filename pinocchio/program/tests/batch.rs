...
fn batch_instruction(instructions: Vec<Instruction>) -> Result<Instruction, ProgramError> {
    ...
    for instruction in instructions {
        ...
        if instruction.program_id.ne(&tpl_token_interface::ID) {
    ...
    Ok(Instruction {
        program_id: tpl_token_interface::ID,
        data,
        accounts,
    })
}
...
async fn batch_initialize_mint_transfer_close() {
    ...
    let mint_len = tpl_token_interface::state::Mint::LEN;
    let mint_rent = rent.minimum_balance(mint_len);

    let account_len = tpl_token_interface::state::Account::LEN;
    ...
    let initialize_mint_ix = tpl_token_interface::instruction::initialize_mint(
        &TOKEN_PROGRAM_ID,
        &mint_a.pubkey(),
    ...
    let initialize_mint_with_freeze_authority_ix =
        tpl_token_interface::instruction::initialize_mint2(
            &TOKEN_PROGRAM_ID,
            &mint_b.pubkey(),
            &mint_authority.pubkey(),
    ...
    let intialize_owner_a_ta_a = tpl_token_interface::instruction::initialize_account3(
        &TOKEN_PROGRAM_ID,
        &owner_a_ta_a.pubkey(),
        &mint_a.pubkey(),
        &owner_a.pubkey(),
    ...
    .unwrap();
    let intialize_owner_b_ta_a = tpl_token_interface::instruction::initialize_account3(
        &TOKEN_PROGRAM_ID,
        &owner_b_ta_a.pubkey(),
        &mint_a.pubkey(),
        &owner_b.pubkey(),
    ...
    let mint_token_a_to_owner_a = tpl_token_interface::instruction::mint_to(
        &TOKEN_PROGRAM_ID,
        &mint_a.pubkey(),
        &owner_a_ta_a.pubkey(),
        &mint_authority.pubkey(),
    ...
    let transfer_token_a_to_owner_b = tpl_token_interface::instruction::transfer(
        &TOKEN_PROGRAM_ID,
        &owner_a_ta_a.pubkey(),
        &owner_b_ta_a.pubkey(),
        &owner_a.pubkey(),
    ...
    // Close Token A
    let close_owner_a_ta_a = tpl_token_interface::instruction::close_account(
        &TOKEN_PROGRAM_ID,
        &owner_a_ta_a.pubkey(),
        &owner_a.pubkey(),
        &owner_a.pubkey(),
    ...
    let mint_a_account =
        tpl_token_interface::state::Mint::unpack(&mint_a_account.unwrap().data).unwrap();
    ...
    let mint_b_account =
        tpl_token_interface::state::Mint::unpack(&mint_b_account.unwrap().data).unwrap();
    ...
    let owner_b_ta_a_account =
        tpl_token_interface::state::Account::unpack(&owner_b_ta_a_account.unwrap().data).unwrap();
...
async fn batch_transfer() {
    ...
    let instruction = batch_instruction(vec![tpl_token_interface::instruction::transfer(
        &TOKEN_PROGRAM_ID,
        &source_account_key,
...
async fn batch_fail_transfer_checked_with_invalid_program_owner() {
    ...
    let instruction = batch_instruction(vec![tpl_token_interface::instruction::transfer_checked(
        &TOKEN_PROGRAM_ID,
        &source_account_key,
        &native_mint_key,
        &destination_account_key,
...
async fn batch_fail_swap_tokens_with_invalid_program_owner() {
    ...
    let instruction = batch_instruction(vec![
        tpl_token_interface::instruction::sync_native(&TOKEN_PROGRAM_ID, &account_b_key).unwrap(),
        tpl_token_interface::instruction::sync_native(&TOKEN_PROGRAM_ID, &account_c_key).unwrap(),
        tpl_token_interface::instruction::transfer(
            &TOKEN_PROGRAM_ID,
            &account_a_key,
            &account_b_key,
            &authority_key,
...
async fn batch_fail_mint_to_with_invalid_program_owner() {
    ...
    let instruction = batch_instruction(vec![
        tpl_token_interface::instruction::mint_to(
            &TOKEN_PROGRAM_ID,
            &mint_key,
            &account_a_key,
            &authority_key,
...
async fn batch_fail_burn_with_invalid_program_owner() {
    ...
    let instruction = batch_instruction(vec![
        ...
        .unwrap(),
        tpl_token_interface::instruction::burn(
            &TOKEN_PROGRAM_ID,
            &account_a_key,
            &mint_key,
            &authority_key,
