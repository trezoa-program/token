...
regression-%:
	mollusk run-test --proto mollusk --ignore-compute-units $(call make-path,$*)/fuzz/program-mb.so ./target/deploy/$(subst -,_,$(shell toml get $(call make-path,$*)/Cargo.toml package.name)).so $(call make-path,$*)/fuzz/blob $(shell toml get $(call make-path,$*)/Cargo.toml package.metadata.trezoa.program-id)
...
execute-fixtures-%:
	mollusk execute-fixture --ignore-compute-units ./target/deploy/$(subst -,_,$(shell toml get $(call make-path,$*)/Cargo.toml package.name)).so ./target/fixtures $(shell toml get $(call make-path,$*)/Cargo.toml package.metadata.trezoa.program-id)

generate-clients:
	pnpm codoma run --all $(ARGS)
