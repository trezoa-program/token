import { Account, generateKeyPairSigner, none, some } from '@trezoa/kit';
import { createDefaultLocalhostRpcClient } from '@trezoa/kit-plugins';
import test from 'ava';
import { fetchMint, getCreateMintInstructionPlan, Mint, tokenProgram } from '../src';
import { createDefaultTrezoaClient, createDefaultTransactionPlanner, generateKeyPairSignerWithTrz } from './_setup';

test('it creates and initializes a new mint account', async t => {
    // Given an authority and a mint account.
    const client = createDefaultTrezoaClient();
    const authority = await generateKeyPairSignerWithTrz(client);
